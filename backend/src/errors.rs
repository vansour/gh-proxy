...
impl ProxyError {
    ...
    pub fn to_status_code(&self) -> StatusCode {
        match self {
            ...
            ProxyError::UnsupportedHost(_) => StatusCode::FORBIDDEN,
    ...
    pub fn to_user_message(&self) -> String {
        match self {
            ProxyError::MethodNotAllowed(message) => format!(
                "Method Not Allowed\n\n{}\n\nThis proxy only supports read-only GET and HEAD requests.\n",
                message
            ),
            ...
            ProxyError::InvalidGitHubUrl(path) => format!(
                "Invalid Request\n\nThe path '{}' is not a valid proxy target.\n\nSupported formats:\n- https://github.com/owner/repo/blob/branch/path/to/file\n- https://raw.githubusercontent.com/owner/repo/branch/path/to/file\n- github.com/owner/repo/blob/branch/path/to/file\n- /github/owner/repo/branch/path/to/file\n\nOnly hosts allowed by proxy.allowed_hosts can be proxied.\n",
                path
            ),
            ProxyError::UnsupportedHost(host) => format!(
                "Host Not Allowed\n\nThe target host '{}' is not in proxy.allowed_hosts.\n\nUpdate the configuration if this upstream should be proxied.\n",
                host
            ),
...
pub fn classify_proxy_error(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::Http(_) => "upstream",
        ProxyError::TooManyConcurrentRequests | ProxyError::RateLimitExceeded => "rate_limit",
        ProxyError::SizeExceeded(_, _) => "size_exceeded",
        ProxyError::UnsupportedHost(_) => "host_not_allowed",
        ProxyError::MethodNotAllowed(_) => "method_not_allowed",
        ProxyError::InvalidTarget(_)
        | ProxyError::InvalidGitHubUrl(_)
        | ProxyError::GitHubRepoHomepage(_)
        | ProxyError::GitHubWebPage(_)
        | ProxyError::InvalidRequest(_) => "invalid_target",
        ProxyError::ProcessingError(msg) if msg.contains("timeout") => "timeout",
        ProxyError::ProcessingError(_) | ProxyError::HttpBuilder(_) | ProxyError::Cache(_) => {
            "other"
        }
    }
}
