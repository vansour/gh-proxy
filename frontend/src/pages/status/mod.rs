...
use self::cards::{BreakdownCard, StatCard};
use self::formatters::{
    collect_dashboard_errors, format_breakdown_items, format_bytes, format_cache_kind_items,
    format_duration, format_host_list, format_number, format_percent,
};
...
fn StatusDashboard(data: DashboardData) -> Element {
    ...
    let method_items = requests
        ...
        .unwrap_or_default();
    let cache_kind_items = cache
        .map(|cache| format_cache_kind_items(&cache.by_kind))
        .unwrap_or_default();
    ...
    rsx! {
        div {
                ...
                StatCard {
                    ...
                    hint: cache
                        .map(|cache| {
                            format!(
                                "{} 条缓存对象 / {} 次合并",
                                format_number(cache.entry_count),
                                format_number(cache.coalesced)
                            )
                        })
            ...
            section { class: "detail-grid",
                ...
                div { class: "detail-card",
                    ...
                    if let Some(cache) = cache {
                        ...
                        div { class: "detail-row",
                            ...
                            span {
                                class: "detail-value",
                                "{format_number(cache.hits)} / {format_number(cache.misses)}"
                            }
                        }
                        div { class: "detail-row",
                            span { class: "detail-label", "驱逐次数" }
                            span { class: "detail-value", "{format_number(cache.evictions)}" }
                        }
                        div { class: "detail-row",
                            span { class: "detail-label", "合并回源" }
                            span { class: "detail-value", "{format_number(cache.coalesced)}" }
                        }
                        div { class: "detail-row",
                            span { class: "detail-label", "回源进行中" }
                            span { class: "detail-value", "{cache.fills_in_flight}" }
                        }
                    } else {
                ...
                BreakdownCard {
                    title: "缓存分类型命中".to_string(),
                    items: cache_kind_items,
                    empty_message: data.stats_error.clone().unwrap_or_else(|| "暂无缓存命中数据".to_string()),
                }
